...
cursor.execute("CREATE DATABASE IF NOT EXISTS prova_flask")

conexao.database = "prova_flask"
...
sql_update = "UPDATE `prova_flask`.`alunos` SET `nome` = 'cleb' WHERE (`ID_aluno` = '1');"
...
conexao.commit()
'''
sql_delete = "DELETE FROM prova_flask.alunos WHERE ID_aluno IN (%s, %s)"
